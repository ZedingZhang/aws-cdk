...
export class Policy extends PolicyBase {
  ...
  constructor(scope: Construct, id: string, props: PolicyProps) {
    ...
    // protection AWS recommends for bedrock-agentcore). A policy ARN is nested under
    // its policy engine (policy-engine/<engineId>/policy/<policyId>); the service
    // appends a random suffix to the name, so the policy id is matched with a wildcard.
