...
describe('PolicyStatement Builder', () => {
  ...
  describe('When conditions', () => {
    ...
    test('Should support isIn() with set literal (not string)', () => {
      ...
      expect(cedar).toContain('["admin", "engineer", "manager"].contains(principal.role)');
    ...
    test('Should support isIn() with numeric values', () => {
      ...
      const cedar = statement.toCedar();
      expect(cedar).toContain('[1, 2, 3, 5, 8].contains(principal.accessLevel)');
    ...
    test('Should support isIn() with mixed string and numeric values', () => {
      ...
      const cedar = statement.toCedar();
      expect(cedar).toContain('["production", "staging", 1, 2].contains(resource.tags)');
...
describe('Comparison operators on ConditionalPolicyStatement (via .when())', () => {
  ...
  test('contains should produce Cedar contains method (set membership)', () => {
    const cedar = makeStatement()
      .when()
...
describe('injection containment and empty id (R5 fix)', () => {
  ...
  test('rejects an injection payload in a specific principal entity id', () => {
    expect(() => PolicyStatement.permit()
      .forPrincipal('AgentCore::OAuthUser', INJECTION_ID)
      .onAllActions()
  ...
  test('rejects an injection payload in a condition value', () => {
    expect(() => PolicyStatement.permit()
      ...
      .onResourceType('AgentCore::Gateway')
      .when()
      .principalAttribute('department').equalTo('Engineering" || true || context has "x')
      .done()
      .toCedar()).toThrow();
  });
  ...
  test('rejects an injection payload in a resource ARN', () => {
    expect(() => PolicyStatement.permit()
      .forAllPrincipals()
      .onAllActions()
      .onResource('AgentCore::Gateway', INJECTION_ID)
      .toCedar()).toThrow();
  });

  test('rejects an injection payload in an entity type (identifier position)', () => {
    expect(() => PolicyStatement.permit()
      .forPrincipal(INJECTION_ID, 'user')
      .onAllActions()
      .onResourceType('AgentCore::Gateway')
      .toCedar()).toThrow();
  });
  ...
  test('renders a namespaced attribute name via the bracket form (does not throw)', () => {
    const cedar = PolicyStatement.permit()
      .forAllPrincipals()
      .onAllActions()
      .onResourceType('AgentCore::Gateway')
      .when()
      .principalAttribute('cognito:groups').contains('admins')
      .done()
      .toCedar();
  ...
  test('rejects an explicit empty principal entity id, pointing at the type-only form', () => {
    expect(() => PolicyStatement.permit()
      .forPrincipal('AgentCore::OAuthUser', '')
      .onActions(['AgentCore::Action::Get'])
      .onResourceType('AgentCore::Gateway')
      .toCedar()).toThrow(/omit the id/i);
  });

  test('rejects an empty action id', () => {
    expect(() => PolicyStatement.permit()
      .forAllPrincipals()
      .onAction('AgentCore::Action::')
      .onResourceType('AgentCore::Gateway')
  ...
  test('names the field per operand position: set member vs condition value', () => {
    expect(() => PolicyStatement.permit()
      ...
      .onResourceType('AgentCore::Gateway')
      .when()
      .principalAttribute('level').isIn([1.5])
      .done()
    ...
    expect(() => PolicyStatement.permit()
      ...
      .onResourceType('AgentCore::Gateway')
      .when()
      .principalAttribute('level').equalTo(1.5)
      .done()
