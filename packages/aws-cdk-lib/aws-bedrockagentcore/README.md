...
The builder is the safe tier: every value you pass to a builder method is written as a single Cedar string literal, and synthesis fails if a value cannot be represented that way.
...
There is no way to escape one value and keep using the builder: the direct tier replaces a whole statement, and builder methods throw once raw Cedar is set.
