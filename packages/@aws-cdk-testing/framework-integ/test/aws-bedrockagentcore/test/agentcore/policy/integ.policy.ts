...
import { Gateway, GatewayAuthorizer, Policy, PolicyEngine, PolicyStatement, PolicyValidationMode } from 'aws-cdk-lib/aws-bedrockagentcore';
...
// AgentCore service accepts the string-literal encoding the builder produces for a
// realistic principal id. Condition methods (isInRange/contains/isIn/equalTo) are
...
new Policy(stack, 'BuilderRealisticUserPolicy', {
  ...
  statement: PolicyStatement.permit()
    .forPrincipal('AgentCore::OAuthUser', 'us-east-1:9f8e7d6c-5b4a-3210-fedc-ba9876543210')
    .onAllActions()
    .onResource('AgentCore::Gateway', sharedGateway.gatewayArn),
  validationMode: PolicyValidationMode.IGNORE_ALL_FINDINGS,
});
